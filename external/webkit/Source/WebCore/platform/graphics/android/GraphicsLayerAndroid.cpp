...
#if ENABLE(ACCELERATED_2D_CANVAS_ANDROID)
    else if (layer && m_contentLayer->isCanvas()) {
        RenderBox* box = layer->renderBox();
        IntRect contentRect = box->contentBoxRect();
        static_cast<Canvas2DLayerAndroid*>(m_contentLayer)->setContentBoxRect(contentRect);
    }
#endif
...
#if ENABLE(WEBGL) || ENABLE(ACCELERATED_2D_CANVAS_ANDROID)
void GraphicsLayerAndroid::setContentsToCanvas(PlatformLayer* canvasLayer)
{
    if (canvasLayer && m_contentLayer != canvasLayer) {
        // Copy data from the original content layer to the new one
